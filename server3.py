...
async def _broadcast_json(obj: dict):
    ...
    for q in list(_client_queues):
        try:
            ...
            # Client is falling behind. For near-real-time TTS, a backlog of
            # stale audio is worse than a dropped chunk, so make room by
            # discarding the oldest pending item rather than disconnecting
            # the client outright.
            try:
                q.get_nowait()
                q.put_nowait(line)
                LOGGER.warning("client queue full; dropped oldest chunk to stay live")
